...
Rectangle {
    ...
    RowLayout {
        ...
        ProgressBar {
            ...
            value: root.progress
            // Hold the indeterminate paint through the fade-out: opacity
            // animates over 150 ms but a bare `root.indeterminate` flips
            // instantly, exposing an empty determinate (value 0) render
            // under the still-visible, fading bar. Revisit if a
            // determinate progress mode is ever driven.
            indeterminate: root.indeterminate || (!root.busy && opacity > 0)
