...
SpinBox {
    ...
    // live text) so the box never resizes as the value changes. A TextInput's
    // implicitWidth is read-only, so the whole control is sized here.
    // leftPadding/rightPadding already include the base padding and the
    // arrow-indicator widths, so the width is correct whether arrows are
    // shown or not; the background min still floors very short ranges.
    // +6 px caret/edge margin.
    ...
    MouseArea {
        ...
        onWheel: (wheel) => {
            ...
            if (!root.activeFocus) {      // unfocused: leave the wheel to the page
